...
export const getInvocesById = invoiceId => {
  return Promise.resolve(db.invoices.find(({ id }) => id === invoiceId));
};
export const getcustomers = () => {
  return Promise.resolve(db.customers);
};
export const getCustomersById = custemrsId => {
  return Promise.resolve(db.customers.find(({ id }) => id === custemrsId));
};
